      ...
      <table className="caption-bottom text-sm w-full">
        <tbody className="[&_tr:last-child]:border-0">
          {tiles.map((tile, index) => (
            <tr
              key={index}
              className="group h-auto w-full bg-white transition-colors hover:bg-gray-50/80 border-b border-gray-100"
            >
              <td className="pl-4 pr-2 py-4 align-top">
                <div className="flex items-center gap-2">
                  <svg
                    ...
                    className="w-3 h-3 text-gray-400 transition-transform group-hover:text-gray-600"
                  >
                    ...
                    {tile.providers.map((provider) => (
                      ...
                      >
                        <div className="flex items-center gap-1">
                          {
                            providerIcons[
                              provider.name as keyof typeof providerIcons
                            ]
                          }
                          ...
                          <>
                            {provider.result.brandMentioned && (
                              <span className="text-xs text-gray-600">
                                ...
                                provider.result.brandPosition > 0
                                  ? `#${provider.result.brandPosition}`
                                  : "Mentioned"}
                              </span>
                            )}
                            <span
                              className={`text-xs font-medium ${
                                provider.result.sentiment === "positive"
                                  ? "text-green-700"
                                  : provider.result.sentiment === "negative"
                                    ? "text-red-700"
                                    : "text-gray-700"
                              }`}
                            >
