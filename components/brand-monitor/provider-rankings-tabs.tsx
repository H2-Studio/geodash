...
const generateFallbackUrl = (competitorName: string): string | undefined => {
  ...
  if (
    cleanName.length < 3 ||
    ["inc", "llc", "corp", "company", "the"].includes(cleanName)
  ) {
    return undefined;
  }
...
export function ProviderRankingsTabs({
  ...
  const brandVisibility =
    competitors.find((c) => c.isOwn)?.visibilityScore || 0;
            ...
            <TabsContent key={provider} value={provider} className="mt-0">
              <div className="overflow-x-auto rounded-lg border border-gray-200">
                <table className="w-full border-collapse">
                  <thead>
                    <tr>
                      <th className="bg-gray-50 border-b border-r border-gray-200 text-left p-3 text-xs font-medium text-gray-900 w-8">
                        #
                      </th>
                  ...
                  <tbody>
                    {competitors.map((competitor, idx) => {
                      const competitorUrl = generateFallbackUrl(
                        competitor.name,
                      );
