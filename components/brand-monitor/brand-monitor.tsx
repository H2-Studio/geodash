...
export function BrandMonitor({ 
  ...
  const [dynamicPromptsError, setDynamicPromptsError] = useState<string | null>(null);
  ...
  const handleScrape = useCallback(async () => {
    ...
    try {
      ...
      if (error instanceof ClientApiError) errorMessage = error.getUserMessage();
      else if (error.message) errorMessage = `Failed to extract company information: ${error.message}`;
  ...
  const handlePrepareAnalysis = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/brand-monitor/check-providers', { method: 'POST' });
      if (response.ok) {
        const data = await response.json();
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    if (onCreditsUpdate) onCreditsUpdate();
    const normalizedPrompts = (state.dynamicPrompts && state.dynamicPrompts.length > 0)
      ...
      : [
          ...
          ].filter((_, index) => !state.removedDefaultPrompts.includes(index)),
          ...state.customPrompts
        ].map(p => p.trim());
  ...
  }, [company, creditsAvailable, onCreditsUpdate, state.dynamicPrompts, state.removedDefaultPrompts, state.customPrompts, startSSEConnection, identifiedCompetitors]);
  ...
  const promptsToShow = (dynamicPrompts && dynamicPrompts.length > 0)
    ? dynamicPrompts
        ...
        <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8">
          {dynamicPromptsLoading ? (
            <div className="flex flex-col items-center justify-center py-8">
              <span className="text-gray-500 text-md mb-2">Generating personalized suggestions...</span>
            </div>
          ) : (
          ...
          {dynamicPromptsError && (
            <div className="text-sm text-red-600 pt-4">{dynamicPromptsError}</div>
          )}
        </div>
      )}
          ...
          <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8">
            <div className="flex gap-6 relative">
              <ResultsNavigation
                ...
                onTabChange={(tab) => dispatch({ type: 'SET_ACTIVE_RESULTS_TAB', payload: tab })}
