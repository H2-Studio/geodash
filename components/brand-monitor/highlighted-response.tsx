...
} from "@/lib/text-highlighting-utils";
...
function cleanResponseText(text: string, providerName?: string): string {
  ...
  const commonProviders = ["OpenAI", "Anthropic", "Google", "Perplexity", "Xai"];
...
export function HighlightedResponse({
  ...
  const detectionResults = React.useMemo(() => {
    ...
    if (response.detectionDetails) {
      ...
      // Add brand detection
      if (
        response.detectionDetails.brandMatches &&
        response.detectionDetails.brandMatches.length > 0
      ) {
        results.set(brandName, {
          mentioned: true,
          matches: response.detectionDetails.brandMatches,
          confidence: Math.max(
            ...response.detectionDetails.brandMatches.map((m) => m.confidence),
          ),
        });
      }
      ...
      if (response.detectionDetails.competitorMatches) {
        ...
        if (response.detectionDetails.competitorMatches instanceof Map) {
          response.detectionDetails.competitorMatches.forEach(
            (matches, competitor) => {
              if (matches.length > 0) {
          ...
          Object.entries(response.detectionDetails.competitorMatches).forEach(
            ([competitor, matches]) => {
              if (matches && matches.length > 0) {
                results.set(competitor, {
                  ...
                  matches,
                  confidence: Math.max(
                    ...matches.map((m: any) => m.confidence),
                  ),
                });
              }
            },
